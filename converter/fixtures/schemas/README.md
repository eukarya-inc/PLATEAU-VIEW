...
is, with i-UR resolved through a relative path inside the package and CityGML
referenced remotely at `schemas.opengis.net`.

They are copied unchanged from
`https://www.geospatial.jp/iur/schemas/<module>/4.0/` and compiled into the
binary, so conversion never touches the network. Fetching would make output
depend on a server, and the published files are updated in place.
