...
# ===== 設定 =====
CI_WORKFLOW_NAME="⭐️ Deploy PLATEAU Server dev"  # 待機するワークフロー
TARGET_BRANCH="main"                               # CIを見るブランチ
DISPATCH_WORKFLOW_FILE="deploy-server-prod.yml"   # 成功時に叩きたい workflow ファイル名
POLL_INTERVAL=10                                   # 何秒おきにチェックするか
MAX_CHECKS=60                                      # 最大チェック回数 (10秒x60=600秒=10分)

echo "Watching workflow '$CI_WORKFLOW_NAME' on branch '$TARGET_BRANCH'..."
...
for i in $(seq 1 "$MAX_CHECKS"); do
  echo "[$i/$MAX_CHECKS] Checking latest workflow run..."
  ...
  # completed かつ success → dispatch
  if [[ "$CONCLUSION" == "success" ]]; then
    echo ""
    echo "✅ Dev workflow succeeded for $SHA."
    echo ""
    ...
    echo ""
    echo "❌ Workflow completed but not successful (conclusion=$CONCLUSION). Aborting."
    exit 1
  fi
done

echo ""
echo "⏰ Timeout: Workflow did not complete successfully within limit (${MAX_CHECKS} checks)."
exit 1
