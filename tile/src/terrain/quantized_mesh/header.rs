...
impl QuantizedMeshHeader {
    ...
    /// [`Self::from_bounds_with_vertices`] for a tighter occluder.
    ...
    /// Like [`Self::from_bounds`], but uses the supplied mesh vertices
    /// (geodetic `(lon, lat, height)`) to compute a tighter horizon
...
/// Compute horizon occlusion point in ellipsoid-scaled coordinates, using
/// the Cesium `EllipsoidalOccluder` algorithm.
///
/// The point lies along the bounding-sphere-center direction (in scaled
/// space). Its magnitude is the max, over every sample on or above the
/// tile, of the "tangent-from-horizon" magnitude that keeps that sample on
/// the camera-side of the horizon plane. For tiles big enough that some
/// sample is orthogonal (or beyond) the direction (e.g. a level-0
/// hemisphere), the magnitude diverges — matching Cesium World Terrain's
/// huge level-0 occlusion points and ensuring those tiles are never
/// falsely culled.
fn compute_horizon_occlusion_point(
    ...
    if dir_mag < 1e-10 {
        // Degenerate (sphere center near origin) — use the +Z pole as a
        // safe default. The visibility test will fall back to frustum
        // culling.
        return [0.0, 0.0, 1.0];
    }
    ...
    if vertices_geodetic.is_empty() {
        // Fallback: corner + edge midpoints at both height extremes, so
        // `from_bounds()` stays standalone-correct for tests.
...
/// Per-vertex magnitude from Cesium's
/// `EllipsoidalOccluder.computeMagnitude`.
///
/// Returns the magnitude (along `direction`, normalized in scaled space)
/// at which the horizon-occlusion point must sit for `position` to stay on
/// the camera-side of the horizon plane. Returns `None` when the formula
/// is numerically unusable (denominator <= 0) — callers should treat that
/// the same as an infinite magnitude (the sample is "off-axis" enough
/// that no finite occluder can shadow it).
fn compute_occlusion_magnitude(position: &[f64; 3], direction: &[f64; 3]) -> Option<f64> {
    ...
    // ellipsoid (e.g. minimum-height samples near the geoid) still
    // produce a valid candidate — Cesium's "PossiblyUnderEllipsoid"
    // variant does this.
...
mod tests {
    ...
    /// Regression: the level-0 eastern-hemisphere tile must produce a
    /// horizon occlusion point that keeps cameras anywhere over the
    /// eastern hemisphere from being culled. Pre-fix, the magnitude was
    /// ~2.4 along +Y, which false-culled cameras with small ECEF Y
    /// (Geneva, Amsterdam, …) because Cesium's
    /// `isScaledSpacePointVisible` test marked the tile as below horizon.
    /// The fix uses Cesium's per-vertex algorithm; for hemisphere tiles
    /// its formula diverges (matching Cesium World Terrain's huge level-0
    ...
    fn test_horizon_occlusion_visible_for_low_longitude_cameras() {
        ...
        // hemisphere we need `P · V > V · V` (condition 1 trivially true
        // with negative vtDotVc).
