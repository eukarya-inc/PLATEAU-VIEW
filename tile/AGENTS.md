...
`/terrain/` (quantized-mesh-1.0, **TMS Geodetic** for Cesium), plus `/terrarium/` (Mapzen Terrarium) and `/mapbox/` (Mapbox Terrain-RGB) endpoints are built-in. Each raster endpoint also exposes a `/tilejson.json` for use as a MapLibre `raster-dem` source. They serve **Web Mercator XYZ** tiles per their respective specs — same projection as a normal MapLibre/Mapbox raster source — at `TERRAIN_TILE_SIZE` (default 256) per side. DEM defaults to Mapterhorn (512 px Terrarium WebP) and is composed with a `japan-geoid` model (GSIGEO2011 / JPGEO2024 / JPGEO2024+Hrefconv) to produce **ellipsoidal heights**. For zooms above the upstream `DEM_MAX_ZOOM`, both the raster endpoints and the Cesium quantized-mesh endpoint fall back to the parent DEM tile and bilinear-upsample the relevant sub-region, so MapLibre's terrain mesh and Cesium's terrain LOD both stay dense at high camera zooms. Tiles outside the configured geoid coverage respond 404.

The geoid **model** is a property of the DEM source, not of the request: a model is bound to a vertical datum (GSIGEO2011 ↔ JGD2011, JPGEO2024 ↔ JGD2024), so each `type: "dem"` source declares `"geoid": "..."` in the config JSON, falling back to `TERRAIN_DEFAULT_GEOID` (default `gsigeo2011`). Models are never mixed and never fall back to one another. What a request selects is the height **mode**, via `?heights=`: `ellipsoidal` (default, orthometric + geoid), `orthometric` (DEM as-is), or `geoid` (the geoid surface alone). The retired `?geoid=<model>` parameter now returns **400** naming the valid `heights=` values — deliberately loud, never silently ignored.
...
- `TERRAIN_TILE_SIZE` (default 256), `TERRAIN_DEFAULT_GEOID` (fallback model for DEM sources that don't declare one; not request-overridable), `TERRAIN_MAX_ZOOM` (default 18; raster endpoints upsample beyond `DEM_MAX_ZOOM`), `TERRAIN_MAX_ERROR`

Cache keys and ETags include the DEM version, upstream ETag digest, geoid-model slug, height-mode slug, and (for Terrarium) output size. The model stays in the key because a source's configured `geoid` can change on a config reload without the DEM slug/version moving; the mode is there because one source now serves three surfaces from identical DEM input. Switching mode does **not** require a CDN purge — each mode lives at a separate key.
...
Terrain source code lives in `src/terrain/` (DEM providers + async tile fetching) and `src/server/terrain.rs` (handlers). The heavy lifting is provided by the [`terrain-codec`](https://crates.io/crates/terrain-codec) crate (≥0.3.0, re-exports the [`martini`](https://crates.io/crates/martini) and [`quantized-mesh`](https://crates.io/crates/quantized-mesh) crates): one-shot heightmap → quantized-mesh encoding (`terrain::encode_terrain_from_fn` — martini RTIN, quantisation, header, vertex normals, gzip), web-mercator → geodetic reprojection (`mercator::MercatorDem`), Terrarium/Mapbox heightmap codecs, tile-coordinate math (`tile_coords`), and `layer.json` types. What remains local is the async DEM orchestration (parallel XYZ fetches, NaN-fill for failures, ETag/timing aggregation in `src/terrain/geodetic.rs`) and DEM-sample sanitisation against corrupt COG values (`src/terrain/mesh_gen.rs`). Build & ship a Japan-only PMTiles mirror with `scripts/japan-pmtiles/`.
