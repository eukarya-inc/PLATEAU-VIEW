...
func (r *mutationResolver) UpdateWorkspace(ctx context.Context, input gqlmodel.UpdateWorkspaceInput) (*gqlmodel.UpdateWorkspacePayload, error) {
	...
	res, err := usecases(ctx).Workspace.Update(ctx, wid, input.Name, nil, getAcOperator(ctx))
	if err != nil {
		return nil, err
	}
...
// RemoveIntegrationsFromWorkspace is the resolver for the removeIntegrationsFromWorkspace field.
func (r *mutationResolver) RemoveIntegrationsFromWorkspace(ctx context.Context, input gqlmodel.RemoveIntegrationsFromWorkspaceInput) (*gqlmodel.RemoveIntegrationsFromWorkspacePayload, error) {
	...
	integrationsIds, err := gqlmodel.ToIDs[accountdomain.Integration](input.IntegrationIds)
	if err != nil {
		return nil, err
	}
	res, err := usecases(ctx).Workspace.RemoveIntegrations(ctx, wId, integrationsIds, getAcOperator(ctx))
	if err != nil {
		return nil, err
	}

	return &gqlmodel.RemoveIntegrationsFromWorkspacePayload{Workspace: gqlmodel.ToWorkspace(res)}, nil
}
